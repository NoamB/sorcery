...
describe SorceryController do
  ...
  describe SorceryController, "with activity logging features" do
    ...
    it "should log last activity time when logged in" do
      ...
      now = Time.now.utc
      get :some_action
      User.first.last_activity_at.utc.to_s.should >= now.utc.to_s
      User.first.last_activity_at.utc.to_s.should <= (now.utc+2).to_s
    end
    ...
    it "'current_users' should hold the user object when 1 user is logged in" do
      ...
      subject.current_users[0].should == User.find(@user.id)
    end
    ...
    it "'current_users' should show all current_users, whether they have logged out before or not." do
      ...
      subject.current_users[0].should == User.find(user1.id)
      subject.current_users[1].should == User.find(user2.id)
      subject.current_users[2].should == User.find(user3.id)
    end
